...
def get_adj_mask(args, data, split_edge, posneg_split):
    path_mask = get_path(args, f'adj_mask_hop{args.mask_hop}_neg{args.mask_neg}', posneg_split)
    no_reproduce = True
    ...
    if osp.exists(path_mask) and no_reproduce:
        ...
        num_neg = int(num_nodes*args.mask_neg)
        if num_neg>0:
        ...
        if num_neg > 0:
            ...
            v.append(np.ones(len(neg_edges[0])) + args.mask_neg_dist)
...
class Distance_edges(Dataset):
    def __init__(self, args, data, split_edge, use_val=False):
        ...
        ##################################################################
        self.num_loop = self.args.max_dist - 2
    ...
    def __getitem__(self, index):
        ...
        for posneg_split in self.posneg_splits:
            ...
            ## impantort: do not change the order of if sentences below
            if self.args.use_dist:
                heuristics.append(A[source, target])
            if self.args.use_cn:
                heuristics.append(Cneis[source, target])
            if self.args.use_ja:
...
def get_dist_edges(args, data, split_edge, posneg_split):
    ## path
    name = f'dist_edges'
    if args.use_dist: name += f'_spd{args.max_dist}'
    if args.use_cn: name += f'_cn'
    if args.use_ja: name += f'_ja'
    if args.use_aa: name += f'_aa'
    if args.use_ra: name += f'_ra'
    name += f'_neg{args.neg_size}'
    path = get_path(args, name, posneg_split)
    ## load data
    no_reproduce = True
    ...
    if args.use_cn:
        cn = raw_edges[i]
        cn[cn > args.max_cn] = args.max_cn
        raw_edges[i] = cn
        i += 1
    if args.use_ja:
        ja = raw_edges[i]*args.mag_ja
        ja[ja > args.max_ja] = args.max_ja
        raw_edges[i] = ja
        i += 1
    if args.use_aa:
        aa = raw_edges[i] * args.mag_aa
        aa[aa > args.max_aa] = args.max_aa
        raw_edges[i] = aa
        i += 1
    if args.use_ra:
        ra = raw_edges[i] * args.mag_ra
        ra[ra > args.max_ra] = args.max_ra
        raw_edges[i] = ra
        i += 1
...
class graph_prepare():
    def __init__(self, args, posneg_split):
        ...
        self.x = self.data.x
        adj_mask = get_adj_mask(self.args, self.data, self.split_edge, self.posneg_split)
    ...
    def resample_edges(self):
        if self.cnt >= self.cnt_max:
            self.cnt = 0
            if self.args.use_dist or self.args.use_cn or self.args.use_ja or self.args.use_aa or self.args.use_ra:
                if self.args.dist_reuse:
                    print('re-use dist edges for neg samples')
                    self.args.dist_reproduce = False
                    self.edges_neg = get_edges(self.args, self.data, self.split_edge, self.posneg_split)
                    self.edges_neg = self.shuffle_edges(self.edges_neg)
