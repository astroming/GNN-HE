...
def get_adj_mask(args, data, posneg_split):
    path_mask = get_path(args, f'adj_mask_hop{args.mask_hop}', posneg_split)
    no_reproduce = True
    if args.mask_reproduce:
        if posneg_split == 'pos_train' or (posneg_split == 'pos_test' and args.use_val):
            no_reproduce = False

    if osp.exists(path_mask) and no_reproduce:
        adj_mask = torch.load(path_mask)
    else:
        ...
        adj_mask['adj'] = adj.copy()
        # high-hop adj for high-hop mask #####################################################################
        if args.mask_hop > 1:
            # base adj with weight 1 #########################################################################
            adj.data = np.ones((len(adj.data)), dtype=args.float)
            adj_t = adj.transpose()
            As, high_adjs = [], {}
            for i in range(args.mask_hop):
        ...
        i = np.vstack((row, col))
        mask = torch.sparse_coo_tensor(i, v, (num_nodes, num_nodes)).coalesce()
        ## to torch_sparse.SparseTensor
        indices, values = mask.indices(), mask.values().float()
        mask = torch_sparse.SparseTensor(row=indices[0], col=indices[1], value=values, sparse_sizes=(mask.size(0), mask.size(1)), is_sorted=True)
        mask.storage.rowptr()
        mask.storage.csr2csc()
        adj_mask[f'mask'] = mask
        ## add self connection to 1-hop adj ######################################################################
        adj_mask['adj'] = adj_mask['adj'] + ssp.identity(adj.shape[0], dtype=args.float)

        # saving high_adjs data ############################################################################
        print(f'saving adj_mask data to {path_mask}')
        torch.save(adj_mask, path_mask, pickle_protocol=4)
...
class Distance_edges(Dataset):
    def __init__(self, args, data, split_edge, use_val=False):
        ...
        ##################################################################
        self.num_loop = self.args.dist_max - 2
        self.posneg_splits, self.edges_dict = self.get_splits_edges()
        self.adj = self.get_adj()
        self.adj_t = self.adj.transpose()
        adj_log = 1 / np.array(np.log(self.adj.sum(axis=1)))
        adj_log = np.nan_to_num(adj_log, nan=0, posinf=1.5, neginf=0).flatten()
        adj_log = self.adj.multiply(adj_log).tocsr()
        self.adj_log = adj_log.transpose()
        step, end = self.args.dist_batch_size, self.adj.shape[0]
    ...
    def __getitem__(self, index):
        ...
        AAdar = (A.dot(self.adj_log) * 10).astype(int)
        AAdar = AAdar.toarray()
        AAdar[AAdar > self.args.aa_max] = self.args.aa_max
        for i in range(self.num_loop):
            # with Timing(name='Times of eli: '):
            A = A.dot(self.adj_t)
            if i == 0:
                Cneis = A.toarray()
                Cneis[Cneis>self.args.cn_max] = self.args.cn_max
        ...
        for posneg_split in self.posneg_splits:
            ...
            spd = A[source, target]
            cn = Cneis[source, target]
            aa = AAdar[source, target]
            dist_edges[posneg_split] = np.vstack((source + perm[0], target, spd, cn, aa)).astype(source.dtype)
...
def get_dist_edges(args, data, split_edge, posneg_split):
    ##
    path = get_path(args, f'dist_edges_spd{args.dist_max}_cn{args.cn_max}_aa{args.aa_max}', posneg_split)
    no_reproduce = True
    if args.dist_reproduce:
        if posneg_split == 'pos_train' or (posneg_split == 'pos_test' and args.use_val):
            no_reproduce = False
    ## get dist_edges #####################################################################
    if osp.exists(path) and no_reproduce:
    ...
    #######################################################################################
    edges = torch.from_numpy(dist_edges[posneg_split])
...
def get_edges(args, data, split_edge, posneg_split):
    posneg, split = posneg_split.split('_')
    if args.use_dist or args.use_cn or args.use_aa:
...
class graph_prepare():
    def __init__(self, args, posneg_split):
        ...
        self.x = self.data.x
        adj_mask = get_adj_mask(self.args, self.data, self.posneg_split)
        self.mask = adj_mask['mask']
        self.degree = np.asarray(adj_mask['adj'].sum(axis=1))
        self.degree[self.degree > args.degree_max] = args.degree_max
        ## edges ############################################################
        ...
        if posneg_split != 'neg_train':
            ...
            ## get neg edges
            if self.args.use_dist or self.args.use_cn or self.args.use_aa:
            ...
            ## resample setting
            self.epoch_neg = int(min(train_size, self.args.batch_size * (self.args.batch_num + 1)))
            self.cnt_max = int(self.edges_neg.size(0) / self.epoch_neg)
            self.cnt = 0
    ...
    def resample_edges(self):
        if self.cnt >= self.cnt_max:
            ...
            if self.args.use_dist or self.args.use_cn or self.args.use_aa:
                ...
                if self.args.dist_fake:
                    edges = get_negative_sampling(self.args, self.data, self.split_edge, num_neg_samples=self.num_neg)
                    perm = np.random.choice(self.dist_cn.size(0), edges.size(1))
                    dist_cn = self.dist_cn[perm]
                    self.edges_neg = torch.cat((edges.t(), dist_cn), dim=1)
                else:
                    self.args.dist_reproduce = True
            ...
            # self.edges_neg = self.shuffle_edges(self.edges_neg)
            self.cnt_max = int(self.edges_neg.size(0) / self.epoch_neg)
        self.edges = self.edges_neg[self.epoch_neg * self.cnt:self.epoch_neg * (self.cnt + 1)]
        self.cnt += 1
