...
class Predictor(nn.Module):
    def __init__(self, args):
        ...
        self.emb_dist = nn.Embedding(args.dist_max + 1, args.dim_encoding)
        self.emb_cn = nn.Embedding(args.cn_max + 1, args.dim_encoding)
        self.emb_aa = nn.Embedding(args.aa_max + 1, args.dim_encoding)
        self.emb_degree = nn.Embedding(args.degree_max + 1, args.dim_encoding)
        ...
        dim_hidden += args.dim_encoding* sum([args.use_dist, args.use_cn, args.use_aa])
    ...
    def forward(self, graph, edge_batch):
        ## input config #####################################
        edge_batch = edge_batch.to(self.args.device)
        feats = graph.x
        ...
        if self.args.use_dist:
            x= torch.cat([x, self.emb_dist(edge_batch[:, 2])], dim=1)
        if self.args.use_cn:
            x = torch.cat([x, self.emb_cn(edge_batch[:, 3])], dim=1)
        if self.args.use_aa:
            x = torch.cat([x, self.emb_aa(edge_batch[:, 4])], dim=1)
        ## link prediction
