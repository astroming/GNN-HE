...
## feature max
parser.add_argument('--dist_max', type=int, default=5)
parser.add_argument('--cn_max', type=int, default=50)
parser.add_argument('--aa_max', type=int, default=50)
parser.add_argument('--degree_max', type=int, default=100)
# distance setting
...
parser.add_argument('--dist_fake', type=bool, default=True)
parser.add_argument('--neg_size', type=float, default=200, help="size of data_neg_train: neg_size * len(split_edge['train']['edge'])")
# mask setting
parser.add_argument('--mask_reproduce', type=bool, default=True, help="whether to reproduce mask")
parser.add_argument('--mask_hop', type=int, default=2, help="neighbor hop. if 1: vanilla GCN. if >1: multiple hop GNN.")
parser.add_argument('--mask_atten', type=str, default='Multiply', help="attention mechnism. options: Concat, Cosine, Multiply, no_atten")
parser.add_argument('--mask_weight', type=str, default='decay', help="weight of mask. options: decay, same, weight (use the original weight)")
...
parser.add_argument('--dim_encoding', type=int, default=1)
...
parser.add_argument('--clip_grad_norm', type=float, default=3.0, help="whether to use clip_grad_norm_ in training")
...
############################################################################################################################
args.dist_max = max(args.dist_max, 3) if args.use_dist else 3 # 3: includes: 2-hop neighbors is 2 and others is 3
...
def test(args, predictor, data_pos, data_neg):
    ...
    def get_predictions(args, predictor, data_pred):
        ...
        num_edge = edges.size(0)
        step, end = args.batch_size*1000, num_edge
        perms = [np.array(range(i, i + step)) if i + step < end else np.array(range(i, end)) for i in range(0, end, step)]
        for perm in perms:
